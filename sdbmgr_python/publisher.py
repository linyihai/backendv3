...
def upload2clod(localfile, serverpath, dstfilename):
    ...
    # print(serverpath)
    # print(dstfilename)
    logger.debug('upload2clod start')
    buff = StringIO()
    c = pycurl.Curl()
    c.setopt(c.URL, serverpath)
    c.setopt(c.WRITEDATA, buff)
    c.setopt(c.HTTPPOST, [('sdb', (c.FORM_FILE, localfile, c.FORM_FILENAME,
                                   dstfilename, c.FORM_CONTENTTYPE, "application/octet-stream")), ])
    c.perform()
    c.close()
    print(buff.getvalue())
...
def map_country(country):
    table = {"中国": 1, "其他": 6, "印尼": 5, "台湾": 2, "新加坡": 5, "日本": 4, "未知": 5,
             "欧美": 3, "泰国": 5, "澳大利亚": 3, "菲律宾": 5, "越南": 5, "韩国": 4, "香港": 2, "马来西亚": 5}
    if country in table:
...
def map_language(lang):
    table = {"国语": 1, "粤语": 2, "闽南语": 3, "英语": 4, "日语": 5, "韩语": 5, "越南语": 6, "菲律宾语": 6, "印尼语": 6, "泰国语": 6,
             "马来西亚语": 6, "意大利语": 7, "客家语": 3, "澳大利亚": 4, "印度语": 7, "法语": 7, "其它": 0, "方言": 3, "西班牙语": 7}
    if lang in table:
        return table[lang]
    else:
        return 0
...
def snap_songdb(conn, songlistid):
    ...
    conn_dst.commit()
    cur_dst.execute(
        "CREATE INDEX [Singer_Country_Index] ON [Singer]([Country] COLLATE [BINARY] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Singer_Pinyin_Index] ON [Singer]([Pinyin] COLLATE [NOCASE] ASC)")
    cur_dst.execute(
        "CREATE UNIQUE INDEX [Singer_index] ON [singer]([SingerNo])")
    cur_dst.execute(
        "CREATE INDEX [Singer_Gender_Index] ON [Singer]([Gender] COLLATE [BINARY] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Singer_hotValue_Index] ON [Singer]([HotValue] COLLATE [BINARY] DESC)")
    cur_dst.execute(
        "CREATE INDEX [Singer_PYStr_Index] ON [Singer]([PYStr] COLLATE [NOCASE] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Singer_SearchLevel_Index] ON [Singer]([SearchLevel] COLLATE [BINARY] ASC)")
    conn_dst.commit()
    ...
    conn_dst.commit()
    cur_dst.execute(
        "CREATE INDEX [Song_PYStr_Index] ON [song]([PYStr1] COLLATE [NOCASE] ASC)")
    cur_dst.execute(
        "CREATE INDEX [song_searchlevel] ON [song]([SearchLevel] COLLATE [BINARY] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Song_HotValue_index] ON [song]([HotValue] COLLATE [BINARY] DESC)")
    cur_dst.execute(
        "CREATE INDEX [Song_Name] ON [song]([Name] COLLATE [NOCASE] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Song_Pinyin_Index] ON [song]([Pinyin] COLLATE [NOCASE] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Song_Language_index] ON [song]([Language] COLLATE [BINARY] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Song_Singer1ID_index] ON [song]([Singer1ID] COLLATE [BINARY] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Song_album_index] ON [song]([album] COLLATE [NOCASE] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Song_Singer2ID_index] ON [song]([Singer2ID] COLLATE [BINARY] ASC)")
    cur_dst.execute(
        "CREATE INDEX [Song_Singer2_index] ON [song]([Singer2] COLLATE [NOCASE] ASC)")
    cur_dst.execute("CREATE UNIQUE INDEX [Song_index] ON [song]([SongNo])")
    cur_dst.execute(
        "CREATE INDEX [Song_Singer1_index] ON [song]([Singer1] COLLATE [NOCASE] ASC)")
    conn_dst.commit()
    logger.debug('create {} ok'.format(swsong))
    logger.debug('insert data to {} start'.format(swsong))
    ...
    while (row):
        ...
        cur_dst.execute(sql_, (singerid, buffer(name.encode('utf8')),
                               pinyin_first, pinyin, song_count, sex, country))
        row = cur.fetchone()
    conn_dst.commit()
    ...
    while (row):
        ...
        if isinstance(row[2], unicode):
            pinyin_first = row[2].encode("utf-8")
        else:
            pinyin_first = row[2]
        if isinstance(row[3], unicode):
            pinyin = row[3].encode("utf-8")
        else:
            pinyin = row[3]
        lang = map_language(row[4])
        if isinstance(row[5], unicode):
            album = row[5].encode("utf-8")
        else:
            album = row[5]
        if isinstance(row[6], unicode):
            singer1 = row[6].encode('utf-8')
        else:
            singer1 = row[6]
        if isinstance(row[7], unicode):
            singer2 = row[7].encode('utf-8')
        else:
            singer2 = row[7]
        singer1id = row[8]
        singer2id = row[9]
        ...
        sql_ = "insert into song(SongNo, Name, PYStr1, Pinyin, Length, Singer1, Singer1ID, Singer2, Singer2ID, album, language, \
                Valid, IsHot, IsNew, VideoType, VideoQaulity, ChannelA, ChannelD, ChannelAVolume, ChannelDVolume, TrackCount) values \
                (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0, 0, ?, ?, ?, ?, ?, ?, ?)"
        if (not singer1):
            singer1 = ""
        if (not singer2):
            singer2 = ""
        cur_dst.execute(sql_, (songid, buffer(name), buffer(pinyin_first), buffer(pinyin), int(duration), buffer(singer1),
                               singer1id, buffer(singer2), singer2id, buffer(album), int(lang), videotype, videoquality,
                               channela, channeld, vol_a, vol_d, trackcount))
        row = cur.fetchone()
    conn_dst.commit()
    cur_dst.close()
    conn_dst.close()
...
def add_songlist2mid(conn, songlistid, mid):
    ...
    if rows:
        ...
        if mid_row["dst_songlist"]:
            ...
            logger.debug("minik[{}] dst_songlist exists, dst_songlist is {}".format(mid, dst_songlist))
            ...
            if allpubsonglist:
                for psl in allpubsonglist:
                    ...
                    for psl_imset_id in psl_imset:
                        sql = "select * from t_config_machset where id = '{}'".format(psl_imset_id)
                        sql_r = DBUtil().get_data(sql)
                        if sql_r and sql_r[0]:
                            psl_imset_machset = demjson.decode(sql_r[0]["machset"])
                            if mid in psl_imset_machset:
                                dst_songlist.append(psl_id)
                        else:
                            logger.error('can not found machset where id = {}'.format(psl_imset_id))
            else:
        ...
        logger.warn("mid[%d] add songlist(%d) to sync failed, mid no in sync table\n" % (mid, songlistid))
        return
...
def update_sync(conn, songlistid, inc, excep):
    ...
    mids = []
    for msetid in incs:
        mids = mids + getmids_from_mset(conn, msetid)
    logger.debug("songlist[%s] related mids is %s" % (songlistid, mids))
    tmp = set(mids)
    mids = [i for i in tmp]
    for mid in mids:
...
def publish_songlist(conn, row):
    ...
    sql = "update t_runtime_songlist set swsong_url='%s', swsong_md5='%s', swsong_v2_url='%s', swsong_v2_md5='%s' where id=%d" % (
        swurl, swmd5, swurl_v2, swmd5_v2, songlistid)
    cur.execute(sql)
    conn.commit()
    # update sync
    update_sync(conn, songlistid, im_set, em_set)
    logger.debug("[%s] publish songlist(%d), pubid=%d\n" % (puber, songlistid, pubid))
...
conn = MySQLdb.connect(host=dbhost, port=dbport,
                       user=dbuser, passwd=dbpasswd, db=songdb)
cur = conn.cursor()
...
while (1):
    cur = conn.cursor()
    cur.execute(
        "select pubid, songlistid, include_mset, except_mset, publisher from t_publish_songlist where status = 0")
    rows = cur.fetchall()
    conn.commit()
    for row in rows:
        try:
