...
class MainLogger(object):
    ...
    def __init__(self, name):
        current_path = os.path.join(os.path.dirname(
            os.path.abspath(__file__)), 'syslog')
        ...
        th_all = logging.handlers.TimedRotatingFileHandler(
            os.path.join(current_path, 'timerotating.log'), when='midnight', interval=1, backupCount=7)
        ...
        rh_all = logging.handlers.RotatingFileHandler(
            os.path.join(current_path, 'filerotating.log'), mode='a', maxBytes=2000 * 2000, backupCount=3)
